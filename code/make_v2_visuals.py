...
ax.scatter(allh.lon,allh.lat,s=7,c=MUT,alpha=0.5,label='All hospitals (190)')
...
ax.set_title("The pre-positioning plan: place PANAF-Premium where the burden is\n25 hospitals — 87.5% of the carpet-viper burden brought within reach",**TITLE)
...
ax.axis('off'); ax.set_title("Who the plan covers\n25 northern hospitals bring 87.5% of the carpet-viper burden within reach",**TITLE)
ax.text(0.0,-0.012,"Note: coverage is measured by BURDEN, not land area — the red southern districts are large but carry little\ncarpet-viper burden, which is why 87.5% of burden is covered even though much of the map's area is red.",
...
a1.axvline(25,color=MUT,ls='-',lw=0.9,alpha=.6); a1.axhline(87.5,color=MUT,ls='-',lw=0.9,alpha=.6)
a1.annotate('25 hospitals → 87.5%',xy=(25,87.5),xytext=(19,54),fontsize=10.5,weight='bold',color=INK,
...
ax.set_title('Demand forecast: how many vials each hospital should hold\nTop 15 of 25 — total '+f"{int(plan.vials_year.sum()):,} vials/yr (~${int(plan.procure_usd_yr.sum()):,}/yr)",**TITLE)
