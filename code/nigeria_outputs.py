...
_GH=json.load(open(f"{BASE}/out2/impact_summary.json")); _GH_RATIO=round(S['total_echis_yr']/_GH['total_echis_yr'])  # v0.7.4: read from the Ghana model, not hand-typed
...
<p class=muted>Nigeria's modelled carpet-viper burden is ~{_GH_RATIO}× Ghana's — the same method, a far larger burden. Because Nigeria is hospital-dense, only {S['pct_unreachable']}% of burden is beyond reach of any hospital (vs {_GH['pct_unreachable']}% in Ghana).</p>
...
<div class=callout><b>The urban artifact, quantified — and what it does and does not change.</b> Because urban LGAs inherit their eco-zone's rural rate, the model's single highest-burden unit is <b>{S['urban_artifact']['top_burden_lga']}</b> ({S['urban_artifact']['top_burden_lga_state']}) at ~{S['urban_artifact']['top_burden_lga_env']:,} envenomings/yr — about {S['urban_artifact']['max_over_p95_ratio']}× the 95th percentile. That is an artifact, not a finding: snakebite envenoming is overwhelmingly rural. We show it rather than hide it, and we quantify its consequence: it dominates the map's colour scale, and only <b>{S['urban_artifact']['pct_vials_to_FCT_or_Lagos']}% of the planned vials</b> are placed at facilities located in the FCT or Lagos. What that metric does <i>not</i> capture: demand originating in city LGAs is routed to peri-urban hospitals nearby (Sabon Wuse for the FCT; Dawakin Kudu for Kano city, whose central LGAs resolve to zero population on the coarse raster), so several high-ranked sites carry inherited urban demand. The savanna and Middle-Belt weight of the plan is real; the exact site ranking near large cities is not, and is finalist work (1-km population raster, rural weighting as in the India model). <b>The direction is robust to the artifact; the near-city rankings are not</b> — which is why we report the placement with this caveat rather than the raw burden raster alone.</div>
<div class=note><b>Known limits of this plan, stated (v0.7.4).</b> (1) Nigeria's existing designated snakebite treatment centres — Kaltungo General Hospital and JUTH Comprehensive Health Centre, Zamko — are in the facility list but are <i>not</i> seeded as fixed sites; the greedy optimiser placed Kumo General Hospital (28 km from Kaltungo) and Bene Hospital (36 km from Zamko) instead. A programme would seed them first; the finalist plan does. (2) EchiTAbG and EchiTAb-Plus-ICP, the products the federal programme has supplied to those centres, are graded in the coverage matrix (grade A and B for <i>E. ocellatus</i>) but are not yet evaluated as the placed product; this plan is costed on PANAF-Premium. Substituting an <i>Echis</i>-adequate product changes cold-chain and cost, not the coverage geometry. (3) The worst-case comparator (0% coverage) is an assumption about a product without <i>Echis</i> efficacy, not an audit of what Nigerian shelves hold today.</div>
...
<footer><b>{VERSION_TAG}</b> — {VERSION_NOTE}<br><br>Same West-African coverage matrix and placed product (PANAF-Premium — WHO risk-benefit-assessed, broad, heat-stable/lyophilised) as the Ghana demonstrator; see the known limits above for the products and treatment centres not yet in the menu. Real inputs: 774 LGAs (<a href="https://www.geoboundaries.org/">geoBoundaries</a>, CC BY 4.0); population <a href="https://www.worldpop.org/">WorldPop</a>/afripop2020 distribution (CC BY 4.0) → 206.1 M (World Bank 2020); {S['n_hospitals']:,} hospitals (Maina 2019 Sci Data 6:134 / WHO). Impact anchored to Habib 2015 PLoS NTD 9(1):e0003381; national mortality GBD 2019 (Nat Commun 2022) and Habib 2015 PLoS NTD 9(9):e0004088; incidence range Habib 2013 J Venom Anim Toxins 19:27. Illustrative feasibility demonstrator — not clinical guidance.</footer></div></body></html>"""
...
<div class=kpis><div class=kpi><b>{O['pct_protected']}%</b><span>carpet-viper burden within reach (versus a worst case of ~0%)</span></div>
