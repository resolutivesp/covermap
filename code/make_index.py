...
import json, os
from viz_common import BASE_CSS, b64, VERSION, VERSION_DATE, VERSION_TAG, VERSION_NOTE
...
<div class=nav>
 <a href="ghana.html"><b>Ghana →</b><span>Placement: 25 named hospitals, 4,654 vials/yr, ~$372k. The best-sourced demonstrator.</span><span class=go>Open the brief</span></a>
...
<div class=grid3>
 <div class=cty><h3>GHANA · PLACEMENT</h3>
  ...
  <div class=lab>of the carpet-viper burden brought <b>within reach</b> of a species-appropriate antivenom, via {GH['optimized']['hospitals']} hospitals (versus a worst case of ~0% under a product without <i>Echis</i> efficacy)</div>
...
<div class=note><b>Artifacts are disclosed and quantified, not hidden.</b> Urban units inherit their zone's rural incidence, which overstates city burden. In Nigeria we show the consequence: only {NG['urban_artifact']['pct_vials_to_FCT_or_Lagos']}% of vials are placed at facilities in the FCT or Lagos, but city demand is routed to peri-urban sites, so the direction of the plan is robust and the near-city rankings are not (stated on the Nigeria brief). In India we fixed it at the source, weighting mortality 94% rural per the Million Death Study.</div>
...
<h2>Verification record — 319 checks, six suites</h2>
...
<tr><td class=l>verify_ghana.py</td><td>66</td><td class=ok>PASS</td><td class=l>population vs census, burden re-derivation, optimiser, demand arithmetic, mortality ceiling, artifact consistency, citations, coverage-matrix rules on the data file (no label-only cell is 'covered'; optimiser flags equal the matrix), cost = whole vials × price</td></tr>
...
<tr><td class=l>verify_india.py</td><td>74</td><td class=ok>PASS</td><td class=l>state rates locked to the published MDS table verbatim, exact state populations, 94/6 rural weighting, ordinal-tier disclosure, published sub-national anchors carried</td></tr>
...
<tr><td class=l>verify_crosscountry.py</td><td>88</td><td class=ok>PASS</td><td class=l>shared parameters identical, honesty invariants hold everywhere, no stale or contradictory numbers — including the methods page and README against the model, no 'from ~0% today' or 'assessment terminated' claim in any brief body, every downloadable CSV stamped</td></tr>
